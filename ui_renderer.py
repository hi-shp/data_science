...
class EnvRenderer:
    def __init__(self, env):
        ...
        # 120 FPS 초고속 렌더링을 위한 게이지 슬라이스 인덱스 테이블 사전 연산
        n_slices = 180
        slice_angles = np.linspace(-np.pi/2, np.pi/2, n_slices)
        cam_w, cam_h = 320, 220
        self._cached_gauge_slices = []
        for i in range(n_slices):
            ang = slice_angles[i]
            idx = int((ang + np.pi) / (2 * np.pi) * 180) % 180
    ...
    def render(self, hits_x, hits_y):
        ...
        fps_lbl = self.get_text_surf(self.fps_font, fps_str, (230, 235, 245))
        fps_sh = self.get_text_surf(self.fps_font, fps_str, (10, 15, 25))
    ...
    def _draw_2d_world(self, hits_x, hits_y, sx, cam_x, target_surf=None):
        ...
        for w in env.wakes:
            ...
            if w[3] > 0:
                ...
                wsy = int(w[1])
                if -50 < wsx < env.w + 50:
                    wake_draw_list.append((wsx, wsy, int(w[2]), w[3]))
        ...
        if all_wake_pts:
            ...
            for wsx, wsy, rad, alpha in wake_draw_list:
                pygame.draw.circle(env.wake_surf, (220, 240, 255, int(alpha * 0.42)), (wsx, wsy), rad)
                if rad > 1.8:
                    pygame.draw.circle(env.wake_surf, (255, 255, 255, int(alpha * 0.72)), (wsx, wsy), int(rad * 0.52))
            for item in rw_draw_list:
                if item[0] == 'circle':
        ...
        if not is_lt and getattr(env, 'show_control_path', True):
            ...
            if path is not None:
                if path is not getattr(self, '_draw_path_source', None):
                    ...
                    self._draw_path_world = path * env.dynamics.pixels_per_m
                if len(path) > 1:
        ...
        # Optional debug overlay reads the actual raw A* route.
        if not is_lt and getattr(env, 'show_raw_route', False) and getattr(env, 'raw_route', None) is not None:
            pts = [(int(sx(x * env.dynamics.pixels_per_m)), int(y * env.dynamics.pixels_per_m)) for x, y in env.raw_route]
            if len(pts) > 1:
                pygame.draw.lines(target_surf, (160, 180, 255), False, pts, 2)
    ...
    def _draw_dashboard(self, hits_x, hits_y, sx=None):
        ...
        if is_lt:
            ...
            for rect, row, field, label, color in options:
                hover = row.collidepoint(mpos)
                if hover:
                    pygame.draw.rect(env.screen, (28, 56, 88), row, border_radius=4)
                pygame.draw.rect(env.screen, (255, 255, 255), rect, 2)
                if getattr(env, field, False):
                    pygame.draw.rect(env.screen, color, rect.inflate(-6, -6))
                env.screen.blit(self.get_text_surf(self.font, label, color if hover else (255, 255, 255)),
                                (70, rect.centery - 10))
    ...
    def _draw_bezier_profile(self, x=None, y=None):
        """Active guidance route in vessel coordinates with yaw-rate reference."""
        env = self.env
        bw, bh = 190, 220
        ...
        if path is not None and len(path) >= 4:
            ...
            # Active guidance is a free-space polyline, not a cubic polynomial.
            formula_txt = f"Yaw ref: {math.degrees(getattr(env, 'command_yaw_rate', 0.)):+.1f} deg/s"
        else:
            sy_max = self.scale_y_max
            sx_max = self.scale_x_max
            surf.blit(self.small_font.render(f"+{sy_max:.1f}", True, (160, 200, 230)), (2, gy - 2))
    ...
    def _draw_telemetry(self):
        ...
        if is_paused:
            ...
            mode_txt = self.get_text_surf(self.bold_font, "PREDICTIVE", (0, 255, 220))
        else:
        ...
        speed_knots = speed / 50.0 * 1.94384
        ...
        speed_ratio = float(np.clip(speed / (50.0*env.dynamics.cruise_speed_m_s), 0.0, 1.0))
    ...
    def _draw_leaderboard_modal(self):
        ...
        # 세련된 무광 딥 슬레이트 배경 + 절제된 서브틀 테두리
        pygame.draw.rect(modal_surf, (16, 20, 26, 252), (0, 0, mw, mh), border_radius=10)
        pygame.draw.rect(modal_surf, (48, 58, 72), (0, 0, mw, mh), 1, border_radius=10)
        ...
        tbl_w = mw - 60
        tbl_h = 295
        pygame.draw.rect(modal_surf, (18, 23, 30, 210), (30, tbl_y, tbl_w, tbl_h), border_radius=6)
        pygame.draw.rect(modal_surf, (36, 45, 58), (30, tbl_y, tbl_w, tbl_h), 1, border_radius=6)

        # 테이블 헤더 행
        th_h = 28
        pygame.draw.rect(modal_surf, (25, 32, 42), (30, tbl_y, tbl_w, th_h), border_top_left_radius=6, border_top_right_radius=6)
        ...
        col_x = 44
        for name, w in cols:
            lbl = self.ko_bold_font.render(name, True, (135, 150, 170))
        ...
        for idx, entry in enumerate(top_10):
            ...
            if is_ai:
                ...
                row_bg = (20, 25, 33, 160) if rank_num % 2 == 1 else (16, 21, 28, 160)
                row_border = None
                t_col = (185, 195, 208)
